"""
File used to run the predictive model API on the LaMP_2 dataset and get the predictions.
...
def extract_predictions(input_string):
    ...
    try:
        ...
        categories_found = [data[sample] for sample in data]
...
# def find_category_in_string(input_string, batch_index, index_in_batch):
...
#     for category in ["women", "religion", "politics", "style & beauty", "entertainment", "culture & arts", "sports", "science & technology", "travel", "business", "crime", "education", "healthy living", "parents", "food & drink"]:
#         if category in input_string:
#             found_category = category
#             break
    
#     if found_category:
#         class_category = found_category
#     else:
#         logger.warning(f"No class category found for sample {index_in_batch} in batch {batch_index}. Appending the class category -1.")
#         class_category = "-1"
...
def append_predictions(saved_outputs, predictions, uids, evaluation_state, batch_index, nb_samples_in_batch):
    ...
    for index_in_batch, category_predicted in enumerate(predictions):

        # sanity check
        # check if the predicted category is a substring of one of the possible categories, and if it is, replace it with the full category name
        for category in categories:
            if category_predicted in category:
                category_predicted = category
                break
        # predictions[index_in_batch] = category_predicted
        ...
        if category_predicted in categories:
            saved_outputs.append(category_predicted)
            evaluation_state['predicted_uids'].append(uids[batch_index * len(predictions) + index_in_batch])
        
        else:
            logger.warning(f"Model output for sample {index_in_batch} in batch {batch_index} is not a valid category. Appending a category -1.")
...
def run_api(total_nb_samples, uids, input_data_batches, saved_outputs, evaluation_state, retry_formatting=False):
    ...
    for batch_index, input_batch in enumerate(input_data_batches):
        ...
        if nb_samples_in_batch == 1:
            full_prompt = f""". 
            You are given a sample for which you have to predict a single class category among those given below.
            You have to respond in json format.
            The key of the json is sample_id and the associated value is the predicted class category.
            The sample is contained in the triple backticks.
            ```{input_batch_concatenated}```
            """
        else:
            full_prompt = f"""
            You are given several samples for which you have to predict a single class category among those given below.
        ...
        if retry_formatting:
            ...
            while not(response.startswith("Sample")) or response.count("Sample") != nb_samples_in_batch:
                ...
                if not(response.startswith("Sample") and response.count("Sample") != nb_samples_in_batch):
                    if wrong_formatting_attempts >= 5:  # we set the predictions for this batch to a class category -1 for all samples in this batch
                        response = "\\n".join([f"Sample {id} category: -1" for id in range(nb_samples_in_batch)])
                    else:
