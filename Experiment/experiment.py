...
def get_dataset_splits(data_folder_path, dataset_version):
    ...
    if "LaMP_8" in dataset_version:
        return None, pd.read_json(data_path + "_en_val_input.json"), None
    ...
    # TODO: change this to "val.json" when the dataset is ready
    val_dataset = pd.read_json(
        data_path + "_dataset_" + "val_subset.json", orient='records')
...
def compute_metrics(gt_targets, predictions):
    ...
    if config.lamp_dataset_index in ["1", "2"]:
        logger.info(
            f"Accuracy score: {accuracy_score(gt_targets, predictions)}")

    elif config.lamp_dataset_index in ["3", "8"]:
        logger.info(
            f"Accuracy score: {accuracy_score(gt_targets, predictions)}")
        logger.info(
            f"Mean Absolute Error (MAE): {mean_absolute_error(gt_targets, predictions)}")
        logger.info(
            f"Root Mean Squared Error (RMSE):  {mean_squared_error(gt_targets, predictions, squared=False)}")
...
def batch_samples(dataset_df):
    logger.info(
        f"Using batches of size {config.request_batch_size} for the API requests.")

    # batch the samples to reduce the total number of API requests to perform
    input_data_batched = [dataset_df['input'][i:i+config.request_batch_size]
                          for i in range(0, len(dataset_df['input']
...
def predict(dataset_df, dataset_version, evaluation_state):
    # Crop dataset.
    n_samples = len(dataset_df['input']
                    ) if not config.n_samples else config.n_samples
    dataset_df = dataset_df[:n_samples]

    logger.info(
        f"We consider the following dataset/dataframe of shape {dataset_df.shape}.")
    ...
    uids = dataset_df['id'] if "LaMP_8" in dataset_version else dataset_df['uid']

    input_data_batched = batch_samples(dataset_df)

    logger.info(
        f"total_number_samples: {total_nb_samples} \n uids: {uids} \n input_data_batched: {input_data_batched} \n outputs: {outputs} \n evaluation_state: {evaluation_state} \n")

    if "LaMP_1" in dataset_version:
        outputs, evaluation_state = model_api_lamp1.run_api(
            total_nb_samples, uids, input_data_batched, outputs, evaluation_state, retry_formatting=False)

    elif "LaMP_2" in dataset_version:
        outputs, evaluation_state = model_api_lamp2.run_api(
            total_nb_samples, uids, input_data_batched, outputs, evaluation_state, retry_formatting=False)

    elif "LaMP_3" in dataset_version:
        outputs, evaluation_state = model_api_lamp3.run_api(
            total_nb_samples, uids, input_data_batched, outputs, evaluation_state, retry_formatting=False)

    elif "LaMP_8" in dataset_version:
        outputs, evaluation_state = model_api_lamp8.run_api(
            total_nb_samples, uids, input_data_batched, outputs, evaluation_state, retry_formatting=False)
...
def save_results_to_file(gt_targets, predictions, evaluation_state, dataset_version):
    ...
    if "LaMP_1" in dataset_version:
        ...
        gt_targets = list(
            map(lambda integer_value: str(integer_value), gt_targets))
        predictions = list(
            map(lambda integer_value: str(integer_value), predictions))
...
def evaluate(data_folder_path, dataset_df, dataset_name, dataset_version, test_mode=False):
    ...
    if config.use_llama_index:
        if config.use_already_indexed_dataset and os.path.exists(f"{data_folder_path}LaMP_{config.lamp_dataset_index}_dataset_{dataset_name}_llama_indexed.json"):
            dataset_df = pd.read_json(
                f"{data_folder_path}LaMP_{config.lamp_dataset_index}_dataset_{dataset_name}_llama_indexed.json", orient='records')
        else:
            dataset_df = llama_index_input_prompts(
                data_folder_path, dataset_df, dataset_name)

    try:
        predictions, evaluation_state = predict(
            dataset_df, dataset_version, evaluation_state)

    # OAI API rate limit is reached
    except (openai.error.RateLimitError, revChatGPT.typings.APIConnectionError) as e:
    ...
    if not test_mode:
        ...
        if 'LaMP_8' in dataset_version:
            gt_targets = pd.read_json(
                f'./Experiment/Data/lamp_reformatted/LaMP_8/LaMP_8_en_val_output.json')['golds'].tolist()[:n_samples]
            gt_targets = [a['review_rating'] for a in gt_targets]
        else:
        ...
        save_results_to_file(gt_targets, predictions,
                             evaluation_state, dataset_version)
        ...
        if "LaMP_1" in dataset_version:
            ...
            predictions = [int(prediction)
                           for prediction in predictions if prediction != '']
            gt_targets = [int(gt_target) for gt_target in gt_targets]
        ...
        logger.info(
            f"Computing metrics for a total of {len(gt_targets)} samples...")
        compute_metrics(gt_targets, predictions)

    else:
        logger.info(
            "Test mode. Hence, no ground truth targets available (None)\n")
        gt_targets = None
...
def setup_experiment():
    ...
    if not config.USE_OAI_ACCESS_TOKEN and not config.USE_OAI_API_KEY and not config.API_CHOICE == "churchless":
        logger.warning(
            "Either an API token or API private key must exist. The flag USE_OAI_ACCESS_TOKEN or USE_OAI_API_KEY should be set.")
    ...
    if not config.lamp_8_samples_version:
        ...
        dataset_version = "LaMP_" + config.lamp_dataset_index + \
            "_" + config.lamp_8_samples_version
...
if __name__ == '__main__':
    ...
    logger.info("\nUsing the following INITIAL config: " +
                config.__str__() + "\n")

    data_folder_path, dataset_version = setup_experiment()

    logger.info("\nUsing the following UPDATED config: " +
                config.__str__() + "\n")
    ...
    train_df, val_df, test_df = get_dataset_splits(
        data_folder_path, dataset_version)

    # train the predictive model
    # trained_model = train(train_df)

    # evaluate the predictive model on the validation set
    gt_targets, outputs, evaluation_state = evaluate(
        data_folder_path, val_df, "val", dataset_version, test_mode=False)
