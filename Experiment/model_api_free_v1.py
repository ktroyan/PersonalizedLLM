...
def append_predictions(saved_outputs, batch_output, uids, uid_index, evaluation_state, batch_index):
    ...
    for index_in_batch, sample in enumerate(batch_output):
        ...
        if review_score in [-1, 1, 2, 3, 4, 5]:
            ...
            evaluation_state['predicted_uids'].append(uids[uid_index])
        else:
...
def get_predictions_from_api_output(saved_outputs, raw_output, uids, uid_index, evaluation_state, batch_index):
    ...
    saved_outputs, evaluation_state = append_predictions(saved_outputs, batch_output, uids, uid_index, evaluation_state, batch_index)
    return saved_outputs, evaluation_state
...
def run_api(total_nb_samples, uids, input_data_batches, saved_outputs, evaluation_state):
    ...
    for batch_index, input_batch in enumerate(input_data_batches):
        ...
        if retry_formatting:
            ...
            while not(output.startswith("Sample")) or output.count("Sample") != nb_samples_in_batch:
                ...
                try:
                    ...
                    nb_tokens_in_llm_prompt = nb_tokens_in_string(nb_tokens_in_llm_prompt, encoding_name="gpt-3.5-turbo")
                    ...
                    llm_index_span = wb_setup.trace_tree.Span(name="llm", span_kind = wb_setup.trace_tree.SpanKind.TOOL)
        ...
        saved_outputs, evaluation_state = get_predictions_from_api_output(saved_outputs, output, uids, uid_index, evaluation_state, batch_index)
