...
def extract_predictions(input_string):
    ...
    try:
        ...
        logger.warning("LLM didn't return a valid JSON.")
...
def append_predictions(saved_outputs, predictions, uids, evaluation_state, batch_index, nb_samples_in_batch):
    ...
    for index_in_batch, review_score in enumerate(predictions):
        ...
        logger.debug(
            f"While appending predictions\n predicted uids: {evaluation_state['predicted_uids']} \n batch index: {batch_index} \n all uids: {uids}\n uid index: {evaluation_state['uid_index']}")

        logger.debug(
            f"Getting uid predicted at position: {batch_index * len(predictions) + index_in_batch}")

        if review_score in [1, 2, 3, 4, 5]:
            saved_outputs.append(review_score)
            evaluation_state['predicted_uids'].append(
                uids[batch_index * len(predictions) + index_in_batch])
        else:
            logger.warning(
                f"Model output for sample {index_in_batch} in batch {batch_index} is not a valid score. Appending a review score of -1.")
...
def get_predictions_from_api_output(saved_outputs, raw_output, uids, evaluation_state, batch_index, nb_samples_in_batch):
    ...
    saved_outputs, evaluation_state = append_predictions(
        saved_outputs, predictions, uids, evaluation_state, batch_index, nb_samples_in_batch)
    return saved_outputs, evaluation_state
...
def run_api(total_nb_samples, uids, input_data_batches, saved_outputs, evaluation_state, retry_formatting=False):
    ...
    if os.path.exists(f"./Experiment/Data/lamp_reformatted/LaMP_{config.lamp_dataset_index}/results/predicted_uids.txt"):
        ...
        with open(f"./Experiment/Data/lamp_reformatted/LaMP_{config.lamp_dataset_index}/results/predicted_uids.txt", 'r') as f:
            ...
            already_predicted_uids = [int(uid.strip()) for uid in already_predicted_uids if (
                uid.strip() != '-1' and uid.strip() != '')]
    ...
    for batch_index, input_batch in enumerate(input_data_batches):
        ...
        input_batch = [f"sample_{index}: {text}" for index, text in enumerate(
            input_batch) if uids[batch_index * len(input_batch) + index] not in already_predicted_uids]
            ...
            while not (response.startswith("Sample")) or response.count("Sample") != nb_samples_in_batch:
                ...
                try:
                    if config.API_CHOICE == "oai":
                        ...
                        response = get_response_from_revchatgpt_v1(
                            chatbot, full_prompt)
                    elif config.API_CHOICE == "V3":
                        response = get_response_from_revchatgpt_v3(
                            chatbot, full_prompt)
                    elif config.API_CHOICE == "churchless":
                        response = get_response_from_churchless(full_prompt)

                    nb_tokens_in_llm_prompt = nb_tokens_in_string(
                        full_prompt, encoding_name="gpt-3.5-turbo")
                    nb_tokens_in_llm_response = nb_tokens_in_string(
                        response, encoding_name="gpt-3.5-turbo")
                    logger.info(
                        f"Number of tokens in llm prompt: {nb_tokens_in_llm_prompt}")
                    logger.info(
                        f"Number of tokens in llm response: {nb_tokens_in_llm_response}")

                    llm_index_span = trace_tree.Span(
                        name="llm", span_kind=trace_tree.SpanKind.TOOL)
                    wb_setup.root_span.add_child_span(llm_index_span)
                    llm_index_span.add_named_result({"llm input": full_prompt}, {
                                                    "llm response": response})
                    llm_index_span.attributes = {
                        "prediction format try n°": wrong_formatting_attempts+1}
                    llm_index_span.attributes = {"batch index": batch_index}
                    llm_index_span.attributes = {
                        "nb of samples processed": evaluation_state['nb_samples_processed'] + nb_samples_in_batch}

                    tokens_used = nb_tokens_in_llm_prompt + nb_tokens_in_llm_response
                    llm_index_span.attributes = {
                        "token_usage_one_llm_exchange": tokens_used}
                ...
                if not (response.startswith("Sample") and response.count("Sample") != nb_samples_in_batch):
                    # we set the predictions for this batch to 3 (median possible score)
                    if wrong_formatting_attempts >= 5:
                        response = "\\n".join(
                            [f"Sample {id} score: 3" for id in range(nb_samples_in_batch)])
                    else:
                        logger.warning(
                            f"Wrongly formatted model response: {response}")
                        logger.warning(
                            "Model did not follow the response format. Requesting the model API again...")
        ...
        saved_outputs, evaluation_state = get_predictions_from_api_output(
            saved_outputs, response, uids, evaluation_state, batch_index, nb_samples_in_batch)
        ...
        logger.info(
            f"{evaluation_state['nb_samples_processed']} samples processed out of {total_nb_samples}")
